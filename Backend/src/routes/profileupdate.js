...
router.route('/OwnerProfile').post(function(req, res){
    ...
    var updateOwner = "UPDATE OWNERS SET OWNERNAME = ?, OWNEREMAIL = ?, OWNERMOB = ?, REST_ZIP = ?, CUISINE = ?, description = ?, timings = ? WHERE (OWNEREMAIL = ?)";
    pool.query(updateOwner, [signupData.name, signupData.email, signupData.mob, signupData.restzip, signupData.cuisine, signupData.description, signupData.timings, signupData.email], (err, result1) => {
        if(err){
            console.log(err);
